...
/*adding send_buffer to sw and updating send work pool*/
...
/*initializes rdma work with given parater*/
...
/* Buffer management and used to get next message from the queue*/
...
/*using the incoming msg calles __put_send_work*/
...
/*check whether message is sent or not*/
...
/*calls function __get_send_work_map in order to send the msg and then marks the messag as done */
...
/*send work is returned to completion handler*/
...
/*frees memory of handle variable*/
...
/*get the memory usong ib_dma_map_single and writes on that memory
	ib_map_mapping_error checks the memory returned is by above function is correct or not
*/
...
/*called when message is done and put back the received work*/
...
/*here received message is send to pcn_kmsg_process*/
...
/*if message is done then complete is called else againsend to __put_send_work*/
...
/*prints which type of work is it*/
...
/*handle the work and calls the function accordingly*/
...
/*create completion queue and queue pairs*/
...
/*inititalizes receive buffer and then
populate receive buffer and work_request*/
...
/*
  ...
  allocates memory using pages and use those mappings to setup rdma buffers
...
/*
just to find out which type of event it belong
*/
...
routing ram and finally connects*/
...
/*
calls function to initialize
setup buffers
calls rdma accept for reconnecting
*/
...
/*
change the states to rdma_connected 
...
/*
creation of ids and binding of addr
*/
...
/*cnnects each of the popcorn nodes*/
...
/*frees each of the memory allocated
 during the process*/
