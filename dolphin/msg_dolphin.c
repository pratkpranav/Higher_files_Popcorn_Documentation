...
/*prints info about local_segment handle*/
...
/*same as above just receive in place of send*/
...
/*send interrupt flag if interrupt_number equals
 local send interrupt no
*/
...
/*
same as above but receive in place of send*/
...
/*print info about the dma transfer status of 
the input given*/
...
/*for enquing send_queue struct to the queue*/
...
/*deques an element form the send_wait_q*/
...
/*initialized a queunode id, then created a local 
segment witht that id, then created and intialized 
interrupt and reconnected connect flag*/
static int pcie_send_init(int channel_num)
{
	int status = 0;
	int value = 0;
...
/*same as above but for receiving*/
static int pcie_recv_init(int channel_num)
{
	int status = 0;
	int value = 0;
...
/*remove interrupt and decentrallize path*/
static int pcie_send_cleanup(int channel_num)
{
	int status = 0;
